...
void get_memory_list_count(MemoryPool* mp, mem_size_t* mlist_len) {
    ...
    std::lock_guard<std::recursive_mutex> locker(*mp->lock);
#endif
...
#if defined(_Z_MEMORYPOOL_THREAD_)
	std::lock_guard<std::recursive_mutex> locker(*mp->lock);
#endif
...
#if defined(_Z_MEMORYPOOL_THREAD_)
	mp->lock = new std::recursive_mutex;
#endif
...
int MemoryPoolDestroy(MemoryPool* mp) {
	if (mp == NULL) return 1;
#if defined(_Z_MEMORYPOOL_THREAD_)
	std::unique_lock<std::recursive_mutex> locker(*mp->lock);
#endif
